...
const Hero: React.FC<HeroProps> = ({ dict, lang }) => {
  ...
  return (
    <section className="hero relative py-20 px-4">
      <div className="container mx-auto text-center">
        ...
        <div className="flex justify-center gap-4">
          <Link href={`/${lang}/upload`}>
            <Button size="lg" className="transition-button smoothButton text-lg bg-teal-300 opacity-60 hover:opacity-100 hover:bg-yellow-200 text-teal-800">
              {dict.home.cta.convert}
            </Button>
          </Link>
          <Link href={`/${lang}/explore`}>
            <Button size="lg" variant="outline" className=" smoothButton text-lg hover:bg-pink-500 hover:text-white text-yellow-200">
              {dict.home.cta.explore}
            </Button>
          </Link>
        </div>
        <p className="text-xl text-white mt-8 max-w-2xl mx-auto">
