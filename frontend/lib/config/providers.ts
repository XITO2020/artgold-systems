// frontend/lib/config/providers.ts
export const PROVIDERS = {
  ...
  storage: {
    ...
    // 👇 On remplace "supabase" par "backend"
    backend: {
      name: 'Backend Storage',
      type: 'http',
      config: {
        // URL publique de ton API (déjà utilisée ailleurs)
        baseUrl: process.env.NEXT_PUBLIC_BACKEND_URL,
        // Endpoint d’upload exposé par le backend
        uploadUrl: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/upload`,
        // Base publique pour servir les médias (ou ton CDN si tu en as un)
        publicBaseUrl: process.env.NEXT_PUBLIC_BACKEND_URL,
      }
    }
  },
...
export type StorageProvider = keyof typeof PROVIDERS.storage;     // 'backend' et 's3'
